...
/** Collection of all effects subscribed to dependency and its keys. Dependency -> dependency key -> dependent effects */
...
export interface EffectOptions {
  ...
  immediate: boolean;
  /** Should effect track dependencies. While it set to `false` no one dependency will be tracked */
  shouldTrack?: boolean;
}
 ...
 * While it is active and `shouldTrack` set to `true`, all used dependencies will be tracked by this effect
 */
export class Effect<T> {
  ...
  static get active(): Effect<any> | null {
    return Effect.activeOnes[Effect.activeOnes.length - 1] ?? null;
  }
  ...
  track(dependency: Dependency, dependencyKey: DependencyKey) {
    this.trackForActive(dependency, dependencyKey);
    this.registerEffect(dependency, dependencyKey);
  }

  /** Subscribe current instance to dependency */
  private trackForActive(dependency: Dependency, dependencyKey: DependencyKey) {
  ...
  private registerEffect(dependency: Dependency, dependencyKey: DependencyKey) {
